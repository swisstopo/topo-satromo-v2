...
if "!config_file!"=="" (
    ...
    echo Beispiel: batch_csplus_processor.bat oed_prod_config.py 2024 04
    goto :eof
)
...
for /L %%D in (1,1,!days_in_month!) do (
	...
	set "process_date=!year!-!month!-!day!"
	
	for /f %%A in ('find /c "!process_date!" ^< processing/running_tasks.csv') do set "lines=%%A"
	if !lines!==0 (
	    python satromo_processor.py !config_file! !process_date!
	)
)

echo *********************************************
echo CSPlus Processor fuer !year! !month! abgeschlossen.
echo *********************************************
echo *********************************************<nul