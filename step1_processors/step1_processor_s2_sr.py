...
def process_product_s2_sr(day_to_process: str, collection: str) -> None:
    ...
    for timestamp, file_list in sorted(files_by_timestamp.items()):
        ...
        # Sort with TCI first, then by resolution and band alphabetically: thsi is the order in the UIS
        file_list.sort(key=lambda x: (x['band'] != 'TCI', x['resolution'], x['band']))
    ...
    main_publish_stac_fsdi.publish_to_stac(f"{config.PRODUCT_S2_LEVEL_2A['product_name'].replace('ch.swisstopo.', '')}_mosaic_{timestamp}_metadata.json",timestamp,config.PRODUCT_S2_LEVEL_2A['product_name'],config.PRODUCT_S2_LEVEL_2A['geocat_id'],None,asset_title="Metadata")
